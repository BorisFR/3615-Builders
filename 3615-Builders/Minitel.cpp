...
Minitel::Minitel() : SoftwareSerial(PIN_MINITEL_SERIAL_TX, PIN_MINITEL_SERIAL_RX) {
  init();
}
...
void Minitel::init() {
  /*begin(1200);
  serialprint7(0x1b); // esc
  serialprint7(0x3a); // pro2
  serialprint7(0x6b); // prog
  //serialprint7(0x76); // 4800b
  serialprint7(0x7f); // 9600b (minitel2)
  end();*/
  begin(9600); //, SERIAL_7O1);
  useDefaultColors();
  refreshSettings();
}
...
void Minitel::serialprint7(byte b) {
  ...
  if (i) {
    bitWrite(b, 7, 1); //écriture de la parité
  }
  else {
    bitWrite(b, 7, 0); //écriture de la parité
  }
  write(b); //écriture du byte sur le software serial
}
